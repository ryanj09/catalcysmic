...
def do_cleanup(catalog):
    ...
    for oname in pbar(keys, task_str):
            ...
            for alias in aliases:
                for prefix in prefixes:
                    if (alias.startswith(prefix) and
                        ...
                        source = catalog.entries[name].add_self_source()
                        catalog.entries[name].add_quantity(
                            CATACLYSMIC.RA, ra, source, derived=True)
                        catalog.entries[name].add_quantity(
                            CATACLYSMIC.DEC, dec, source, derived=True)
                        break
        ...
        if (CATACLYSMIC.RA in catalog.entries[name] and
                CATACLYSMIC.DEC in catalog.entries[name] and no_host):
            from astroquery.irsa_dust import IrsaDust
            if name not in catalog.extinctions_dict:
                try:
                    ra_dec = catalog.entries[name][
                        CATACLYSMIC.RA][0][QUANTITY.VALUE] + \
                        " " + \
                        catalog.entries[name][CATACLYSMIC.DEC][0][QUANTITY.VALUE]
            ...
            if name in catalog.extinctions_dict:
                ...
                (catalog.entries[name].add_quantity(
                    ...
                    derived=True))
        if ((CATACLYSMIC.HOST in catalog.entries[name] and
             (CATACLYSMIC.HOST_RA not in catalog.entries[name] or
              CATACLYSMIC.HOST_DEC not in catalog.entries[name]))):
            for host in catalog.entries[name][CATACLYSMIC.HOST]:
                ...
                if ' J' in alias and is_number(alias.split(' J')[-1][:6]):
                    ...
                    source = catalog.entries[name].add_self_source()
                    catalog.entries[name].add_quantity(
                        CATACLYSMIC.HOST_RA, hostra, source, derived=True)
                    catalog.entries[name].add_quantity(
                        CATACLYSMIC.HOST_DEC, hostdec, source, derived=True)
                    break
        ...
        if (CATACLYSMIC.REDSHIFT not in catalog.entries[name] and
            ...
            bestsig = 0
            for hv in catalog.entries[name][CATACLYSMIC.VELOCITY]:
        ...
        if (CATACLYSMIC.REDSHIFT not in catalog.entries[name] and
            ...
            for host in catalog.entries[name][CATACLYSMIC.HOST]:
                if host[QUANTITY.VALUE] in catalog.nedd_dict:
                    ...
                    catalog.entries[name].add_quantity(
                        [CATACLYSMIC.REDSHIFT, CATACLYSMIC.HOST_REDSHIFT],
                        redshift,
        ...
        if (CATACLYSMIC.MAX_ABS_MAG not in catalog.entries[name] and
            ...
            bestsig = 0
            for ld in catalog.entries[name][CATACLYSMIC.LUM_DIST]:
            ...
            if bestsig > 0 and is_number(bestld) and float(bestld) > 0.:
                ...
                bestldz = z_at_value(cosmo.luminosity_distance,
                                     float(bestld) * un.Mpc)
                pnum = (
                    float(catalog.entries[name][CATACLYSMIC.MAX_APP_MAG][0][
                        QUANTITY.VALUE]) - 5.0 *
            ...
            if bestsig > 0 and is_number(bestld) and float(bestld) > 0.:
                ...
                pnum = (
                    float(catalog.entries[name][
                        CATACLYSMIC.MAX_VISUAL_APP_MAG][0][QUANTITY.VALUE]) -
        ...
        if all([
                ...
                for x in [
                    CATACLYSMIC.RA, CATACLYSMIC.DEC, CATACLYSMIC.HOST_RA,
                    CATACLYSMIC.HOST_DEC
                ]
        ]):
            # For now just using first coordinates that appear in entry
            try:
                c1 = coord(
                    ra=catalog.entries[name][CATACLYSMIC.RA][0][QUANTITY.VALUE],
                    dec=catalog.entries[name][CATACLYSMIC.DEC][0][
                        QUANTITY.VALUE],
                    unit=(un.hourangle, un.deg))
                c2 = coord(
                    ra=catalog.entries[name][CATACLYSMIC.HOST_RA][0][
                        QUANTITY.VALUE],
                    dec=catalog.entries[name][CATACLYSMIC.HOST_DEC][0][
                        QUANTITY.VALUE],
                    unit=(un.hourangle, un.deg))
            except (KeyboardInterrupt, SystemExit):
                raise
                ...
                sources = uniq_cdl(
                    ...
                    entries[name][CATACLYSMIC.RA][0][QUANTITY.SOURCE].split(',')
                    + catalog.entries[name][CATACLYSMIC.DEC][0][QUANTITY.SOURCE]
