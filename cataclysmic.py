...
class Cataclysmic(Entry):
    ...
    def __init__(self, catalog, name, stub=False):
        """Initialize `Cataclysmic`."""
        super(Cataclysmic, self).__init__(catalog, name, stub=stub)
        return
            ...
            if quantity == self._KEYS.ALIAS:
                for alias in self.get(quantity, []):
                    ...
                    if (cleaned_value.startswith('CV') and
                        ...
                        success = super(Cataclysmic, self).add_quantity(
                            CATACLYSMIC.ALIAS, 'AT' + cleaned_value[2:], source,
                            **kwargs)
    ...
    def add_source(self, **kwargs):
        ...
        return super(Cataclysmic, self).add_source(**kwargs)
    ...
    def extra_aliases(self):
        ...
        """
        if (self[CATACLYSMIC.NAME].startswith('CV') and
                is_number(self[CATACLYSMIC.NAME][2:6])):
            return ['AT' + self[CATACLYSMIC.NAME][2:]]
        return []
    ...
    def _get_save_path(self, bury=False):
        ...
        cataclysmic.  If `bury` is true, then this entry is saved to the
        'boneyard'.
        """
    ...
    def clean_internal(self, data):
        """Clean input data from the 'Cataclysmic/input/internal' repository.
    ...
    def set_first_max_light(self):
        if CATACLYSMIC.MAX_APP_MAG not in self:
            ...
            if mldt:
                ...
                self.add_quantity(
                    CATACLYSMIC.MAX_DATE, max_date, uniq_src, derived=True)
            if mlmag:
                mlmag = pretty_num(mlmag)
                self.add_quantity(
                    CATACLYSMIC.MAX_APP_MAG, mlmag, uniq_src, derived=True)
            if mlband:
                self.add_quantity(
                    CATACLYSMIC.MAX_BAND, mlband, uniq_src, derived=True)

        if CATACLYSMIC.MAX_VISUAL_APP_MAG not in self:
            ...
            if mlmag:
                mlmag = pretty_num(mlmag)
                self.add_quantity(
                    CATACLYSMIC.MAX_VISUAL_APP_MAG,
                    mlmag,
                    uniq_src,
                    derived=True)
            if mlband:
                self.add_quantity(
                    CATACLYSMIC.MAX_VISUAL_BAND, mlband, uniq_src, derived=True)
    ...
    def purge_bandless_photometry(self):
        ...
        """
        if CATACLYSMIC.PHOTOMETRY not in self:
            return
        mjds = [
            float(x[PHOTOMETRY.TIME]) for x in self[CATACLYSMIC.PHOTOMETRY]
        ...
        maxmjd = max(mjds) + 1
        newphotos = []
        for photo in self[CATACLYSMIC.PHOTOMETRY]:
        ...
        self[CATACLYSMIC.PHOTOMETRY] = newphotos
        return

    def get_best_redshift(self, key=CATACLYSMIC.REDSHIFT):
        bestsig = -1
        bestkind = None
        for z in self[key]:
            try:
    ...
    def set_preferred_name(self):
        """Set preferred name of cataclysmic variable.
        ...
        if not newname and CATACLYSMIC.DISCOVERER in self:
            discoverer = ','.join(
                [x['value'].upper() for x in self[CATACLYSMIC.DISCOVERER]])
